...
// NOTE: BFS and DFS and UCS may not use these functions, just placeholders here in case
struct BFS {
...
struct DFS {
    bool operator()(const State* a) const {
        return a; // UPDATE AS DFS is created
    }
};
...
struct Puzzle {
    ...
    vector<string> calcBFS() {
        cout << "Calculating using BFS!" << endl;

        // Declare data structures and initialize states
        //priority_queue<State*, vector<State*>, Comparator> searchNext;
        queue<State*> searchNext;
        unordered_set<string> visited;    // Use unordered set to achieve 0(1) lookup for visited states using its string
        State* startState = new State(start, nullptr, 0);
        searchNext.push(startState);
    ...
    // ********* Depth First Search Implementation Here **********
    template <typename Comparator>
    vector<string> calcDFS() {
        cout << "Calculating using DFS!" << endl;

        // Declare data structures and initialize states

        // Depth First Search uses a stack data structure
        stack<State*> searchNext;
        unordered_set<string> visited;    // Use unordered set to achieve 0(1) lookup for visited states using its string
        State* startState = new State(start, nullptr, 0);
        searchNext.push(startState);
        ...
        while (!searchNext.empty()) {
            ...
            // Iterate through the neighbors of the blanktile to "consider options" for possible moves
            // Edit for DFS to add children to searchNext, and then explore the top of the stack next
            // More DFS notes: Consider adding neighbors in reverse order
            for (Tile* side : blankTile->sides) {
                State* newState = new State(currString, currState, currState->level + 1); // for further memory management, possibly consider using States directly, not pointers...
                newState->swapSymbols(side->index);
                ...
                if (visited.find(newString) == visited.end()) {  // State has not been visited
                    // Check if state already exists in visited set
                    findDistances(newState);
                    // visited.insert(newString); taken this out for DFS
                    searchNext.push(newState);
                }
                else
                    delete newState;
    ...
    // ******* UCS IMPLEMENTATION HERE **********
    // NOTE: With equal movement cost, UCS and BFS are IDENTICAL...
    // !!!!!!
    // WE CAN REMOVE THIS BECAUSE IT IS IDENTICAL TO BFS HER EUNLESS CHANGES ARE MADE
    // !!!!!!
    template <typename Comparator>
    vector<string> calcUCS() {
        cout << "Calculating using UCS!" << endl;

        // Declare data structures and initialize states
        //priority_queue<State*, vector<State*>, Comparator> searchNext;
        priority_queue<State*> searchNext;
        unordered_set<string> visited;    // Use unordered set to achieve 0(1) lookup for visited states using its string
        State* startState = new State(start, nullptr, 0);
        searchNext.push(startState);
...
vector<string> run(string start, string goal, string algorithm) {
    ...
    if (algorithm == "BFS")
        path = puzzle->calcBFS<BFS>();
    else if (algorithm == "DFS")
        path = puzzle->calcUCS<UCS>();
    else if (algorithm == "UCS")
        path = puzzle->calcUCS<UCS>(); // identical to BFS in unweighted graph...
...
int main() {
    ...
    vector<string> path = run(start, goal, "BFS");
