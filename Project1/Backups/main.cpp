...
struct Puzzle {
    ...
    vector<string> calcPath() {
        // Declare data structures and initialize states
        priority_queue<State*, vector<State*>, AStar> searchNext;
...
vector<string> run(string start, string goal) {
    vector<string> path;
    ...
    // Calculate path (start state -> end state)
    path = puzzle->calcPath();
...
int main() {
    ...
    string start = "THISISARAND_OMPZ";
    string goal =  "THISISARANDOMPZ_";

    vector<string> path = run(start, goal);
...
PYBIND11_MODULE(pathfinder, m) {
	...
	m.def("find_path", &run,
		  "Find the shortest path from the start state to a goal state",
