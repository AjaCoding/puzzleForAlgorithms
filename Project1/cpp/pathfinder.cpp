...
struct Puzzle {
    ...
    vector<string> calcWPQ() {
        cout << "Calculating using " << typeid(Comparator).name() << "!" << endl;
...
vector<string> run(string start, string goal, string algorithm) {
    ...
    if (algorithm == "BFS")
        ...
        path = puzzle->calcWPQ<UCS>(); // identical to BFS in unweighted graph...
    else if (algorithm == "Greedy")
        path = puzzle->calcWPQ<Greedy>();
    else if (algorithm == "A*")
        path = puzzle->calcWPQ<AStar>();
    else {
